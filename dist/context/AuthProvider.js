...
function createAdapter(config) {
    if (config.mode === "cookie")
        return createCookieAdapter(config);
    if (config.mode === "api_key")
        return createApiKeyAdapter(config);
    return createOidcAdapter(config);
}
export function AuthProvider(props) {
    const adapterRef = React.useRef(null);
    if (!adapterRef.current)
        adapterRef.current = createAdapter(props.config);
    ...
    return _jsx(AuthContext.Provider, { value: state, children: props.children });
}
